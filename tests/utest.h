...
#ifndef TESTS_UTEST_H_
#define TESTS_UTEST_H_

#define Test(name, suite) \
    void test_run_##name(TestCase *test); \
    TestCase test_##name((const char *)#name, \
        (utest_run_fcn_t)test_run_##name); \
    TestCaseAdder adder_##name(&suite, &test_##name); \
    void test_run_##name(TestCase *test)

#define assertTrue(condition) if(test->_assertTrue(NULL, (condition), __FILE__, __LINE__)) return; // NOLINT

#define assertEquals(expected, actual) if(test->_assertEquals(NULL, expected, actual, __FILE__, __LINE__)) return; // NOLINT

#define assertEqualsMsg(msg, expected, actual) if(test->_assertEquals(msg, expected, actual, __FILE__, __LINE__)) return; // NOLINT
...
int8_t TestCase::_assertTrue(char *msg, bool expected, const char *file,
        int line) {
    if (!expected) {
